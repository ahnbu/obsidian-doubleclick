...
| 일시 | 유형 | 범위 | 변경내용 (목적 포함) |
|---|---|---|---|
| 2026-08-27 15:40 | docs | verify | 직전 커밋의 콜드 스타트 원인 진단 정정 — 옵시디언 전체 종료 후 실측한 결과 창은 elapsed=1.0s attempts=1로 1초 만에 떴다. '창이 뜨기 전에 예산이 만료된다'는 진단은 반증됐고, 체감 지연은 빈 창이 채워지는 시간(인덱싱+플러그인)이다. 과거 실패 23건(1.0%)의 원인은 여전히 미상. 적응형 예산은 성공 시 즉시 반환해 비용이 0이고 exact 우선 정책은 독립적으로 유효하므로 되돌리지 않되, README 설명을 실측에 맞게 교체 |
